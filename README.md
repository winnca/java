...
2. [Описание технологий проекта](#title2)
   ...
   - 2.5. [Жизненный цикл Spring-приложения (общий)](#title25)
   - 2.6. [Жизненный цикл Spring-бина](#title26)
   - 2.7. [Пример для нашего RESTful приложения](#title27)
   - 2.8. [Как Spring обрабатывает HTTP-запрос и отправляет ответ](#title28)
...
### <a id="title25">2.5. Жизненный цикл Spring-приложения (общий)</a>
...
2. **Инициализация Spring Boot**
   ...
   - Создаётся `ApplicationContext` (IoC контейнер)
   - Выполняется настройка автоконфигурации

3. **Жизненный цикл бинов** (подробно в разделе 2.6)
...
5. **Обработка HTTP-запросов** (работа приложения)
   ...
   - `POST /api/students/save_student` — создание студента
   - `PUT /api/students/update_student` — обновление студента
...
7. **Завершение работы бинов**
   - Вызов destroy-методов (если они указаны, например `@PreDestroy`)
...
### <a id="title26">2.6. Жизненный цикл Spring-бина (на примере `StudentController`)</a>

Этот процесс происходит на этапе 3 общей схемы приложения и повторяется для каждого бина в контейнере.

| № | Этап | Описание |
|---|------|----------|
| **1** | **Определение бина** | Spring находит класс с аннотацией `@RestController`. Создаётся `BeanDefinition` — метаданные о бине. |
| **2** | **Создание объекта бина** (Instantiation) | Вызывается конструктор класса: `new StudentController()`. В консоли: "Конструктор StudentController". |
| **3** | **Внедрение зависимостей** (Dependency Injection) | Spring анализирует параметры конструктора, находит готовый бин `StudentService` и внедряет его: `this.studentService = studentService`. |
| **4** | **Вызов init-метода** (`@PostConstruct`) | Если в классе есть метод с `@PostConstruct`, Spring вызывает его. Здесь можно выполнить логику инициализации (проверка БД, загрузка данных). В консоли: "@PostConstruct: бин инициализирован". |
| **5** | **Бин готов к использованию** | `StudentController` может обрабатывать HTTP-запросы: `GET`, `POST`, `PUT`, `DELETE` к эндпоинтам `/api/students`. |
| **6** | **Вызов destroy-метода** (`@PreDestroy`) | При остановке приложения Spring вызывает метод с `@PreDestroy`. Здесь можно освободить ресурсы (закрыть соединения, файлы). В консоли: "@PreDestroy: бин уничтожается". |
| **7** | **Бин уничтожен** | Объект становится доступен для сборщика мусора (Garbage Collector). |

<br>
<br>

### <a id="title27">2.7. Пример для нашего RESTful приложения</a>

#### Запуск приложения

1. JVM стартует
2. Spring Boot инициализируется
3. **Начинается создание бинов (в порядке зависимостей):**
...
5. **Приложение работает** — готово обрабатывать запросы

#### Остановка приложения

1. Получен сигнал на остановку (Ctrl+C)
2. **Начинается завершение бинов (в обратном порядке):**
...
### <a id="title28">2.8. Как Spring обрабатывает HTTP-запрос и отправляет ответ</a>

Процесс обработки входящего HTTP-запроса в Spring MVC:

| № | Этап | Описание |
|---|------|----------|
| **1** | **Клиент отправляет запрос** | Браузер или Postman отправляет HTTP-запрос (например, `GET http://localhost:8080/api/students`) на сервер. |
| **2** | **Запрос попадает в `DispatcherServlet`** | Главный сервлет Spring MVC, который принимает все входящие запросы и выступает в роли диспетчера. |
| **3** | **`DispatcherServlet` обращается к `HandlerMapping`** | `HandlerMapping` хранит соответствия между URL, HTTP-методами и методами контроллеров. Он находит нужный метод (`findAllStudents()`) в нужном контроллере (`StudentController`). |
| **4** | **Вызов метода контроллера** | `DispatcherServlet` вызывает найденный метод, подготавливая аргументы (из `@RequestBody`, `@PathVariable` и т.д.). |
| **5** | **Работа бизнес-логики** | - Контроллер делегирует задачу сервису (`StudentService`).<br>- Сервис содержит бизнес-логику и обращается к репозиторию (`StudentRepository`).<br>- Репозиторий взаимодействует с БД (PostgreSQL), выполняя SELECT, INSERT, UPDATE, DELETE. |
| **6** | **Формирование ответа** | Результат проходит обратный путь: БД → Репозиторий → Сервис → Контроллер. Контроллер возвращает объекты (например, `List<Student>`). |
| **7** | **`DispatcherServlet` использует `HttpMessageConverter`** | Spring автоматически конвертирует Java-объекты в JSON с помощью Jackson (так как контроллер помечен `@RestController`). |
| **8** | **Отправка ответа клиенту** | `DispatcherServlet` отправляет HTTP-ответ (JSON + статус 200 OK) обратно клиенту. |
